    ...
    def _collect_samples(self):
        ...
        while self.running:
            try:
                ...
                warnings.warn(f"Failed to collect power sample: {e}", stacklevel=2)
...
def calculate_power_metrics(power_samples, timestamps, total_tokens, benchmark_duration):
    ...
    if not power_samples or len(power_samples) < 2:
        # Return default values if we don't have enough samples
        return PowerMetrics(
...
async def benchmark(
    ...
    power_monitor = PowerMonitor(interval=0.1).start()
    print("Started GPU power monitoring")
    ...
    power_samples, timestamps = power_monitor.stop()
    print("Stopped GPU power monitoring")
    ...
    total_tokens = metrics.total_input + metrics.total_output
...
def run_process_benchmark(process_input):
    """Run benchmark in a separate process."""
    # Unpack inputs
    backend = process_input["backend"]
    api_url = process_input["api_url"]
    base_url = process_input["base_url"]
    model_id = process_input["model_id"]
    model_name = process_input["model_name"]
    
    # Create a new tokenizer instance in each process instead of using the parent's
    tokenizer_id = process_input["tokenizer_id"]
    tokenizer_mode = process_input["tokenizer_mode"]
    trust_remote_code = process_input["trust_remote_code"]
    tokenizer = get_tokenizer(tokenizer_id, tokenizer_mode=tokenizer_mode, trust_remote_code=trust_remote_code)
    
    chunk_requests = process_input["chunk_requests"]
    logprobs = process_input["logprobs"]
    request_rate = process_input["request_rate"]
    burstiness = process_input["burstiness"]
    disable_tqdm = process_input["disable_tqdm"]
    profile = process_input["profile"]
    selected_percentile_metrics = process_input["selected_percentile_metrics"]
    selected_percentiles = process_input["selected_percentiles"]
    ignore_eos = process_input["ignore_eos"]
    goodput_config_dict = process_input["goodput_config_dict"]
    max_concurrency = process_input["max_concurrency"]
    lora_modules = process_input["lora_modules"]
...
def aggregate_results(results):
    """Aggregate benchmark results from multiple processes."""
    if len(results) == 1:
        return results[0]
    ...
    sum_metrics = [
        ...
        "duration", "total_power_joules"
    ]
    
    # List metrics to extend
    list_metrics = [
        "input_lens", "output_lens", "ttfts", "itls",
        "generated_texts", "errors"
    ]
    
    # Metrics to average (weighted by completed requests)
    avg_metrics = [
        "request_throughput", "request_goodput", "output_throughput",
        "total_token_throughput", "peak_power_watts", "avg_power_watts",
        "mean_tokens_per_watt", "median_tokens_per_watt", "p99_tokens_per_watt"
    ]
    
    # Add all percentile metrics
    for metric in ["ttft", "tpot", "itl", "e2el"]:
        for prefix in ["mean_", "median_", "std_", "p50_", "p90_", "p95_", "p99_"]:
            key = f"{prefix}{metric}_ms"
            if key in aggregated:
                avg_metrics.append(key)
    
    # Process results after the first one
    total_completed = aggregated["completed"]
    for result in results[1:]:
        # Sum metrics
        for metric in sum_metrics:
            if metric in result and metric in aggregated:
                aggregated[metric] += result[metric]
        
        # Extend list metrics
        for metric in list_metrics:
            if metric in result and metric in aggregated:
                aggregated[metric].extend(result[metric])
        
        # Add to completed count for weighted averages
    ...
    if total_completed > 0:
        for result in results:
            weight = result["completed"] / total_completed
            for metric in avg_metrics:
                if metric in result and metric in aggregated:
                    # Initialize if this is the first weighted contribution
                    if result == results[0]:
                        aggregated[metric] = weight * result[metric]
                    else:
                        aggregated[metric] += weight * result[metric]
...
def main(args: argparse.Namespace):
    ...
    # Set the tokenizer environment variable to avoid warnings
    os.environ["TOKENIZERS_PARALLELISM"] = "false"

    # Get the tokenizer in the main process
    ...
    # Split requests for multi-process execution
    num_processes = min(multiprocessing.cpu_count(), 4)  # Use up to 4 CPU cores
    print(f"Using {num_processes} CPU cores for parallel request handling")
    
    chunk_size = len(input_requests) // num_processes
    request_chunks = []
    for i in range(0, len(input_requests), chunk_size):
        end_idx = min(i + chunk_size, len(input_requests))
        request_chunks.append(input_requests[i:end_idx])
    
    # Adjust max_concurrency per process
    process_max_concurrency = None
    if args.max_concurrency:
        process_max_concurrency = max(1, args.max_concurrency // num_processes)

    # Create process inputs for each chunk
    process_inputs = []
    for chunk in request_chunks:
        process_inputs.append({
            ...
            "model_id": model_id,
            "model_name": model_name,
            "tokenizer_id": tokenizer_id,  # Pass ID instead of object
            "tokenizer_mode": tokenizer_mode,  # Pass mode for recreation
            "trust_remote_code": args.trust_remote_code,
            "chunk_requests": chunk,
            "logprobs": args.logprobs,
            "request_rate": args.request_rate / num_processes,  # Adjust rate per process
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        results = list(executor.map(run_process_benchmark, process_inputs))
    
    # Aggregate results from all processes
    benchmark_result = aggregate_results(results)
