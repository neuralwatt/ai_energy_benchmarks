...
def main():
    ...
    if baseline:
        print(f"Memory: {baseline.memory_used_mb:.0f} MB / {baseline.memory_total_mb:.0f} MB ({baseline.memory_percent:.1f}%)")
    ...
    load_result = GPUMonitor.monitor_during_operation(
        load_model,
        gpu_id=0,
        interval=0.5
    )
    ...
    if after_load:
        ...
        print(f"  Memory: {after_load.memory_used_mb:.0f} MB / {after_load.memory_total_mb:.0f} MB ({after_load.memory_percent:.1f}%)")
    ...
    for i, prompt in enumerate(test_prompts):
        ...
        if result['success']:
            ...
            print(f"    GPU Utilization: {gpu_stats['avg_utilization_percent']:.1f}% avg, {gpu_stats['max_utilization_percent']:.1f}% peak")
            print(f"    GPU Memory: {gpu_stats['avg_memory_mb']:.0f} MB avg, {gpu_stats['max_memory_mb']:.0f} MB peak")
            if gpu_stats['avg_power_w']:
                print(f"    Power: {gpu_stats['avg_power_w']:.1f}W avg, {gpu_stats['max_power_w']:.1f}W peak")
    ...
    if after_cleanup:
        ...
        print(f"  Memory: {after_cleanup.memory_used_mb:.0f} MB ({after_cleanup.memory_percent:.1f}%)")
